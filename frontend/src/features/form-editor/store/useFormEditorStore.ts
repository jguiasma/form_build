...
import type { FormSchema, FormStep, FormField, FieldOption } from '../types/form-builder.types';
...
interface FormEditorState {
  ...
  updateBiometricRequirement: (required: boolean) => void;
}
...
export const useFormEditorStore = create<FormEditorState>()(
  persist(
    (set, get) => ({
      ...
      updateBiometricRequirement: (required) => set((state) => ({
        schema: { ...state.schema, require_biometrics: required }
      }))
    }),
    {
