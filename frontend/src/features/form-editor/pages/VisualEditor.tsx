...
const VisualEditor: React.FC = () => {
  ...
  const { schema, setSchema, activeStepId, updateFormDetails, updateBiometricRequirement } = useFormEditorStore();
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="flex flex-col h-screen bg-[#f8fbfe] font-['Inter'] text-slate-800 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-full hover:border-blue-300 transition-all">
              <span className={`material-symbols-outlined text-lg ${schema.require_biometrics ? 'text-blue-600' : 'text-slate-400'}`}>
                {schema.require_biometrics ? 'face' : 'face_off'}
              </span>
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 mr-1">Face ID</span>
              <button 
                onClick={() => updateBiometricRequirement(!schema.require_biometrics)}
                className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors focus:outline-none ${schema.require_biometrics ? 'bg-blue-600' : 'bg-slate-300'}`}
              >
                <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${schema.require_biometrics ? 'translate-x-4' : 'translate-x-1'}`} />
              </button>
            </div>
