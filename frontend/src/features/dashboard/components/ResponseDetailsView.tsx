    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-24 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm flex items-center gap-1.5 ${
              response.status === 'completed' 
               ...
               {response.biometric_verified && <span className="material-symbols-outlined text-xs">verified</span>}
               {response.status === 'completed' ? (response.biometric_verified ? 'BIOMETRICALLY VERIFIED' : 'REVIEWED & VERIFIED') : 'PENDING REVIEW'}
            </span>
            {response.user && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-[#f8faff] p-12 rounded-[3.5rem] border border-blue-100 flex flex-col xl:flex-row items-center gap-10 relative overflow-hidden group">
             <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.06] transition-all">
                <span className="material-symbols-outlined text-[10rem] rotate-12">verified_user</span>
             </div>
             <div className="size-32 bg-white rounded-[2.5rem] shadow-sm flex items-center justify-center border border-blue-50 shrink-0">
                <span className="material-symbols-outlined text-6xl text-[#1148ad] animate-pulse">fingerprint</span>
             </div>
             <div className="flex-1 space-y-5 relative z-10">
                <div className="flex items-center justify-between">
                   <h4 className="text-xl font-black text-slate-900">Biometric Verification</h4>
                   <span className={`flex items-center gap-1.5 text-[10px] font-black uppercase ${response.biometric_verified ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-rose-600 bg-rose-50 border-rose-100'} px-4 py-1.5 rounded-full border shadow-sm`}>
                    <span className="material-symbols-outlined text-xs">{response.biometric_verified ? 'verified' : 'block'}</span>
                    {response.biometric_verified ? 'Verified' : 'Unverified'}
                   </span>
                </div>
                <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-lg">
                   {response.biometric_verified 
                     ? 'AI-enhanced scan completed successfully. Identity confirmed via cryptographic template matching.'
                     : 'This response was submitted without biometric verification or verification failed.'}
                </p>
                {response.biometric_verified && (
                   <div className="flex gap-4">
                      <div className="bg-white p-5 rounded-2xl border border-blue-50 flex-1 shadow-sm">
                         <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Match Confidence</p>
                         <p className="text-2xl font-black text-blue-600">{Math.round((response.biometric_score || 0) * 1000) / 10}%</p>
                      </div>
                      <div className="bg-white p-5 rounded-2xl border border-blue-50 flex-1 shadow-sm">
                         <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Verification ID</p>
                         <p className="text-lg font-black text-blue-600 truncate">VA-{response.id}-BIO</p>
                      </div>
                   </div>
                )}
             </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           ...
           <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm group hover:shadow-md transition-all">
              ...
              <div className="space-y-10 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-px before:bg-slate-100">
                 ...
                 <div className="flex gap-5 relative">
                    <div className="size-6 rounded-full bg-blue-600 border-4 border-white shadow-sm shrink-0 z-10 ring-4 ring-blue-50"></div>
                    <div>
                      <p className="text-[13px] font-black text-slate-900">Biometric Verified</p>
                      <p className="text-[10px] font-bold text-slate-400 mt-1">System (Auto-scan) • {formattedTime}</p>
                    </div>
                 </div>
